...
export const NumerologySection: React.FC = () => {
  ...
  const handleConsultClick = () => {
    trackEvent('numerology_consult_clicked', { source: 'numerology_section' });
  };

  const whatsappUrl = `https://wa.me/${siteConfig.whatsapp}?text=${encodeURIComponent(
    language === 'hi'
      ? 'नमस्ते पंडित जी, मुझे अंकशास्त्र (अंक ज्योतिष) परामर्श के बारे में जानकारी चाहिए।'
      : 'Namaste Pandit Ji, I would like to consult you for Vedic Numerology (Ank Jyotish).'
  )}`;

  return (
    <section className="section-padding bg-ivory relative overflow-hidden" id="numerology-section">
      {/* Background patterns */}
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <ScrollReveal direction="up" delay={0.1}>
            <span className="section-label text-gold justify-center">
              <span aria-hidden="true">🔢</span> अंक ज्योतिष
            </span>
        ...
        <ScrollReveal className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          ...
          <div className="group relative flex flex-col p-8 rounded-2xl bg-white border border-gold/15 hover:border-gold/35 hover:shadow-gold-sm hover:bg-gold/5 transition-all duration-300 shadow-glass">
            ...
            <h3 className="text-xl font-poppins font-bold text-primary mb-3">
              {t('numerology.card1Title')}
            </h3>
            <p className="text-text-muted text-sm leading-relaxed">
              {t('numerology.card1Desc')}
            </p>
          </div>
          ...
          <div className="group relative flex flex-col p-8 rounded-2xl bg-white border border-gold/15 hover:border-gold/35 hover:shadow-gold-sm hover:bg-gold/5 transition-all duration-300 shadow-glass">
            ...
            <h3 className="text-xl font-poppins font-bold text-primary mb-3">
              {t('numerology.card2Title')}
            </h3>
            <p className="text-text-muted text-sm leading-relaxed">
              {t('numerology.card2Desc')}
            </p>
          </div>
          ...
          <div className="group relative flex flex-col p-8 rounded-2xl bg-white border border-gold/15 hover:border-gold/35 hover:shadow-gold-sm hover:bg-gold/5 transition-all duration-300 shadow-glass">
            ...
            <h3 className="text-xl font-poppins font-bold text-primary mb-3">
              {t('numerology.card3Title')}
            </h3>
            <p className="text-text-muted text-sm leading-relaxed">
              {t('numerology.card3Desc')}
            </p>
          </div>
        </ScrollReveal>
