...
for (const [slug, faqs] of Object.entries(faqsData)) {
  const regex = new RegExp(`(slug:\\s*."${slug}"[\\s\\S]*?metaDescription:[\\s\\S]*?),\\s*\\}`, "g");
  content = content.replace(regex, (match, p1) => {
    // Only inject if not already there
    if (match.includes("faqs:")) return match;
    const faqsStr = JSON.stringify(faqs, null, 4).replace(/\\n/g, "").replace(/^/gm, "    ");
    return `${p1},\n    faqs: ${faqsStr.trim()}\n  }`;
  });
}

fs.writeFileSync(filePath, content, "utf8");
console.log("Injected FAQs into services.ts successfully.");
